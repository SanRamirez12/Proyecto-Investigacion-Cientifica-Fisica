'''
Pipeline principal para la clasificacion de fuentes en 4 clases: 
    -BLL
    -FSRQ
    -NoAGN
Las mejores metricas alcanzadas en general fueron: 
Exactitud promedio entre folds: 77.65 ± 2.17 %
F1-score (weighted) promedio entre folds: 0.7981 ± 0.0152
     F1-score promedio por clase entre folds:
     - BLL: 0.8322 ± 0.0201
     - FSRQ: 0.7890 ± 0.0268
     - NoAGN: 0.8135 ± 0.0191
     - OtroAGN: 0.1121 ± 0.0683
AUC promedio por clase en test:
    BLL: 0.9257 ± 0.0078
    FSRQ: 0.9398 ± 0.0146
    NoAGN: 0.9420 ± 0.0163
    OtroAGN: 0.6635 ± 0.0918
'''
...
for fold_idx, (trainval_idx, test_idx) in enumerate(kfold.split(X, Y)):
    ...
    #Se define un test set a partir de los 15%  asignados en el SSS y un 85% a train/val
    X_trainval, X_test = X.iloc[trainval_idx].copy(), X.iloc[test_idx].copy()
    Y_trainval, Y_test = Y[trainval_idx], Y[test_idx]

    #Se realiza una division de los sets train y val internos para cada fold 80% train y 20%val
    ...
    smote_nc = SMOTENC(categorical_features=categorical_indices, #determinamos cuales son las columnas categoricas
                       sampling_strategy={1: 856, 2: 813, 3: 700}, #Estrategia de generacion de datros sinteticos
